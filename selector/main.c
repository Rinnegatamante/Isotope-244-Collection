...
int main(int argc, char *argv[]) {
	...
	GLboolean available_games[3] = {GL_FALSE, GL_FALSE, GL_FALSE};
	GLboolean has_at_least_one_game = GL_FALSE;
	
	for (int i = 0; i < 3; i++) {
		sprintf(fname, "ux0:data/isotope/game%d/libMySDLMainActivity.so", i + 1);
	...
	selector_idx++;
	FILE *f = fopen("ux0:data/isotope.tmp", "wb+");
